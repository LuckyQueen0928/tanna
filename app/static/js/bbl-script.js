...
$(function() {
    ...
    d3.json(config.jsonUrl, function(data) {
        ...
        drawGraph();
        var bw = $(window).width();
        var lw = $("#graph").find("svg").width();
        var bh = $(window).height();
        var lh = $("#graph").find("svg").height();
        $("#graph-container").scrollLeft(Math.abs(lw-bw)/2);
        $("#graph-container").scrollTop(Math.abs(lh-bh)/2);
        
    });

    // close the doc of nodes
...
function drawGraph() {
    ...
    graph.width = $('#graph').width() + 1000 - graph.margin.left - graph.margin.right;
    graph.height = $('#graph').height() + 1000- graph.margin.top  - graph.margin.bottom;

    $('#graph').css('display', display);

    // set obj by for each constraints position linkStrength of each data
    for (var name in graph.data) {
        var obj = graph.data[name];
        obj.positionConstraints = []; //位置约束
        obj.linkStrength        = 1; //边长
    ...
    graph.categoryKeys = d3.keys(graph.categories); //名称数组
    ...
    graph.nodeRect = graph.node.append('rect')
        ...
        .attr('ry', 5)
        .attr('stroke', function(d) {
          if (d.categoryKey == 'view:covered') {
            return '#26D82F';
          }
          if (d.categoryKey == 'view:found') {
            return '#D8CD2E';
          }
          return '#8C8C8C';
        })
    ...
    // var newwidth = graph.width + 1000 + "px";
    // var newheight = graph.height + 100 + "px";
    // $("body").width(newwidth).height(newheight);
    // $("#SVG").width(newwidth);
}
...
function selectObject(obj, el) {
    ...
    $.cookie("bbl-select", obj["name"].split(":")[0],{expires: 7, path: '/features'});
